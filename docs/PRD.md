...
- 编写或修改 JavaScript、TypeScript 时自动触发日常编码模式，但只处理当前任务中新写或已经触及的代码区域，不扫描未触及代码，也不自动纳入任务开始前已有的 Git 工作区改动。
- 日常编码模式从目标文件和执行面收集最小充分的兼容性证据。证据充分时自然采用稳定且兼容的能力；目标环境缺失、冲突或无法可靠定位时静默跳过现代化判断，不阻塞原任务。
- 用户只指定 Skill 而未限定单项能力，或明确要求批量检查冗余、寻找最新语法替代、审查 ES20xx 使用机会时进入显式审查模式。该模式扫描用户指定范围，并在自动发现后集中询问确实会改变结论的缺失约束。
- 日常任务涉及单项 ECMAScript 语法或内置 API 时只做局部兼容性判断，不自动扩大为仓库级审查。
- 显式审查先从 Stage 4 提案中定位待验证候选，再结合 TypeScript、构建目标、Node.js、浏览器、库消费者和部署环境过滤安全能力；日常编码和单项能力判断只采用已经通过兼容性门槛的能力。所有模式都不凭单一工具版本推导能力，也不隐式提高兼容性基线。详细工作流以 [`modernize-ecmascript` Skill](../.agents/skills/modernize-ecmascript/SKILL.md) 为权威入口。
