...
- 从目标文件定位实际解析器、转换器和输出目标，只自动采用能被解析并安全转换的 Stage 4 语法。
- 语法会保留在产物中时，只有存在明确的目标运行时支持证据才采用。
- 默认不自动采用依赖运行时实现、polyfill 或 shim 的标准内置 API。
...
先读取索引，使用其中的标题、阶段、状态、ES 版本和提案页链接定位能力。索引已提供链接时，不要自行拼接页面地址。

- 用户指定某项语法、API、ES 版本或提案时，读取相关提案页面。
...
- 日常模式只修改当前任务新写、正在修改或已经触及的代码区域，不单独发送现代化计划。
