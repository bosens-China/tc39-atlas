import React, { useMemo } from 'react';
import { Link, useLang, useSearchParams } from '@rspress/core/runtime';
import { useQuery } from '@tanstack/react-query';
import { Button, Card, Empty, Radio, Spin, Tag } from 'antd';
import {
  ArrowRightOutlined,
  CheckCircleOutlined,
  ClockCircleOutlined,
  DisconnectOutlined,
  PlusCircleOutlined,
  ReloadOutlined,
  RiseOutlined,
} from '@ant-design/icons';
import {
  fetchDataset,
  selectChanges,
  type ProposalChangeKind,
} from '../api/client';
import { StageTag, StatusTag } from '../components/StageTag';
import { proposalRoutePath } from '../proposalRoute';
import {
  readChangePeriod,
  writeChangePeriod,
  type ChangePeriod,
} from './changePeriod';

export function ChangesPage() {
  const lang = useLang();
  const [searchParams, setSearchParams] = useSearchParams();
  const period = readChangePeriod(searchParams);
  const {
    data: dataset,
    error,
    isPending,
    refetch,
  } = useQuery({
    queryKey: ['dataset'],
    queryFn: ({ signal }) => fetchDataset(signal),
  });
  const data = useMemo(
    () => (dataset ? selectChanges(dataset, period, 500) : undefined),
    [dataset, period],
  );
  const changes = data?.changes ?? [];
  const titleZhById = useMemo(
    () =>
      new Map(
        dataset?.proposals.map((proposal) => [proposal.id, proposal.titleZh]) ??
          [],
      ),
    [dataset],
  );

  const renderKindBadge = (kind: ProposalChangeKind) => {
    const config: Record<
      ProposalChangeKind,
      { color: string; icon: React.ReactNode; label: string }
    > = {
      added: {
        color: 'cyan',
        icon: <PlusCircleOutlined />,
        label: lang === 'zh' ? '新增提案' : 'Added',
      },
      stage_changed: {
        color: 'purple',
        icon: <RiseOutlined />,
        label: lang === 'zh' ? '阶段跃迁' : 'Stage changed',
      },
      finished: {
        color: 'green',
        icon: <CheckCircleOutlined />,
        label: lang === 'zh' ? '完成入规范' : 'Finished',
      },
      withdrawn: {
        color: 'red',
        icon: <DisconnectOutlined />,
        label: lang === 'zh' ? '提案撤回' : 'Withdrawn',
      },
      inactive: {
        color: 'orange',
        icon: <ClockCircleOutlined />,
        label: lang === 'zh' ? '转不活跃' : 'Inactive',
      },
    };

    const { color, icon, label } = config[kind];
    return (
      <Tag
        color={color}
        className="flex items-center space-x-1 px-2.5 py-0.5 text-xs font-semibold rounded-md"
      >
        {icon}
        <span>{label}</span>
      </Tag>
    );
  };

  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      {/* 头部标题与周期切换 */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="mb-2 text-3xl font-extrabold tracking-tight text-slate-950 dark:text-slate-100">
            {lang === 'zh' ? 'TC39 周期变化动态' : 'TC39 proposal changes'}
          </h1>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            {lang === 'zh'
              ? '追踪提案的阶段演进、新增、撤回、不活跃和正式进入规范。'
              : 'Track proposal additions, stage changes, withdrawals, inactivity, and completion.'}
          </p>
        </div>

        <Radio.Group
          value={period}
          onChange={(event) => {
            const nextPeriod = event.target.value as ChangePeriod;
            setSearchParams(writeChangePeriod(nextPeriod), { replace: true });
          }}
          buttonStyle="solid"
          className="bg-slate-800 p-1 rounded-xl border border-slate-700"
        >
          <Radio.Button value="day">
            {lang === 'zh' ? '过去 24 小时' : '24 hours'}
          </Radio.Button>
          <Radio.Button value="week">
            {lang === 'zh' ? '过去 7 天' : '7 days'}
          </Radio.Button>
          <Radio.Button value="month">
            {lang === 'zh' ? '过去 30 天' : '30 days'}
          </Radio.Button>
        </Radio.Group>
      </div>

      {/* 动态内容 */}
      {isPending ? (
        <div className="flex flex-col items-center justify-center py-24 space-y-4">
          <Spin size="large" />
          <span className="text-sm text-slate-500 dark:text-slate-400">
            {lang === 'zh' ? '读取动态数据中...' : 'Loading changes...'}
          </span>
        </div>
      ) : error && !data ? (
        <div className="rounded-xl border border-slate-200 bg-slate-50 py-16 dark:border-slate-700/50 dark:bg-slate-800/40">
          <Empty description={<span>{error.message}</span>}>
            <Button icon={<ReloadOutlined />} onClick={() => void refetch()}>
              {lang === 'zh' ? '重试' : 'Retry'}
            </Button>
          </Empty>
        </div>
      ) : changes.length > 0 ? (
        <div className="space-y-4">
          {changes.map((change) => {
            const time = new Date(change.occurred_at).toLocaleString('zh-CN', {
              month: 'numeric',
              day: 'numeric',
              hour: '2-digit',
              minute: '2-digit',
            });

            return (
              <Card
                key={change.id}
                className="rounded-xl border-slate-200 bg-white/80 transition-all hover:border-indigo-500/40 dark:border-slate-700/70 dark:bg-slate-800/80"
                styles={{ body: { padding: '1.25rem' } }}
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  {/* 左侧：类型徽章与标题 */}
                  <div className="space-y-2">
                    <div className="flex items-center space-x-3">
                      {renderKindBadge(change.kind)}
                      <span className="font-mono text-xs text-slate-500 dark:text-slate-400">
                        {time}
                      </span>
                    </div>

                    <h3 className="text-base font-semibold text-slate-950 dark:text-slate-100">
                      {lang === 'zh'
                        ? (titleZhById.get(change.proposal_id) ??
                          change.after.title)
                        : change.after.title}
                    </h3>
                  </div>

                  {/* 右侧：变更前后对照与查看按钮 */}
                  <div className="flex items-center space-x-4 rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 dark:border-slate-800/80 dark:bg-slate-900/60">
                    {change.before && (
                      <div className="flex items-center space-x-1.5 opacity-70">
                        <StageTag stage={change.before.stage} />
                        <StatusTag status={change.before.status} />
                      </div>
                    )}

                    {change.before && (
                      <ArrowRightOutlined className="text-slate-500 text-xs" />
                    )}

                    <div className="flex items-center space-x-1.5">
                      <StageTag stage={change.after.stage} />
                      <StatusTag status={change.after.status} />
                    </div>

                    <Link
                      to={proposalRoutePath(change.proposal_id, lang)}
                      className="font-medium text-indigo-500 hover:text-indigo-400 dark:text-indigo-400 dark:hover:text-indigo-300"
                    >
                      {lang === 'zh' ? '查看提案' : 'View proposal'}
                    </Link>
                  </div>
                </div>
              </Card>
            );
          })}
        </div>
      ) : (
        <div className="rounded-xl border border-slate-200 bg-slate-50 py-16 dark:border-slate-700/50 dark:bg-slate-800/40">
          <Empty
            description={
              <span className="text-slate-500 dark:text-slate-400">
                {lang === 'zh'
                  ? '所选时间段内暂无变化事件'
                  : 'No changes occurred during this period'}
              </span>
            }
          />
        </div>
      )}
    </div>
  );
}
